...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith('.pdf')) {
      return NextResponse.json({ error: 'Only PDF templates are supported' }, { status: 400 });
    }
    ...
    return await generatePdfDocument(file, fields);
  } catch (err) {
...
async function generatePdfDocument(
  ...
  for (const field of fields) {
    if (!field.value || !field.rectangle) continue;
  ...
  const outName = `filled_${path.basename(file.name, '.pdf')}.pdf`;
...
// ─────────────────────────────────────────────────────────────────────────────
// WORD SUPPORT - PRESERVED FOR FUTURE USE
// The functions below handle Word (.docx) template generation with positional
// text boxes and label-fill patterns. To re-enable Word support:
//   1. Restore the Word branch in the POST handler above
//   2. Accept .docx files and route to generateWordDocument()
//   3. Re-enable parse routes (app/api/try/parse and parse-client)
// ─────────────────────────────────────────────────────────────────────────────
...
//
//   // Pass 0: replace [Name] bracket-style placeholders
//   placeholderMap.forEach((value, placeholder) => {
//     if (!/^\[.+\]$/.test(placeholder)) return;
...
//   });
//
//   // Pass 0b: drawn fields — insert as absolutely-positioned floating text boxes
...
//     execSync(`"${soffice}" --headless --convert-to pdf "${tmpDocx}" --outdir "${tmpDir}"`, { timeout: 30000 });
...
// Suppress unused import warnings for Word support
