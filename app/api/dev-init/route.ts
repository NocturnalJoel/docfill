...
export async function GET(_request: NextRequest) {
  ...
  const res = NextResponse.redirect(new URL('/dashboard/clients', siteUrl));
  res.cookies.set('dev_bypass', '1', { path: '/', httpOnly: true, sameSite: 'lax' });

  const admin = createAdminClient();

  // Ensure dev user exists in auth.users (foreign key requirement)
  const { error: userError } = await admin.auth.admin.createUser({
    id: DEV_USER_ID,
    email: 'demo@paperworkslayer.com',
    email_confirm: true,
    password: crypto.randomUUID(),
  });
  // Ignore "already exists" error (code 422 / message contains "already registered")
  if (userError && !userError.message?.includes('already been registered')) {
    return NextResponse.json({ error: 'Auth user creation failed', detail: userError }, { status: 500 });
  }

  // Wipe existing dev data
  const d1 = await admin.from('client_documents').delete().eq('user_id', DEV_USER_ID);
  const d2 = await admin.from('clients').delete().eq('user_id', DEV_USER_ID);
  const d3 = await admin.from('templates').delete().eq('user_id', DEV_USER_ID);
  if (d1.error || d2.error || d3.error) {
    return NextResponse.json({ error: 'Delete failed', d1: d1.error, d2: d2.error, d3: d3.error }, { status: 500 });
  }

  // ── Clients ──────────────────────────────────────────────────────────────
  const clientIds = {
    mitchell: uuidv4(), okafor: uuidv4(), sharma: uuidv4(),
    chen: uuidv4(), beaumont: uuidv4(), nguyen: uuidv4(),
  };

  const r1 = await admin.from('clients').insert([
  ...
  if (r1.error) return NextResponse.json({ error: 'Clients insert failed', detail: r1.error }, { status: 500 });

  // ── Templates ─────────────────────────────────────────────────────────────
  const templateIds = { nda: uuidv4(), service: uuidv4(), intake: uuidv4(), claim: uuidv4() };

  const r2 = await admin.from('templates').insert([
    {
      id: templateIds.nda, user_id: DEV_USER_ID, name: 'Non-Disclosure Agreement',
      file_name: 'NDA_Template.docx', file_type: 'word', file_url: 'dev/templates/nda.docx',
      uploaded_at: daysAgo(60), page_count: 2,
      fields: [
    ...
    {
      id: templateIds.service, user_id: DEV_USER_ID, name: 'Service Agreement',
      file_name: 'Service_Agreement_Template.docx', file_type: 'word', file_url: 'dev/templates/service.docx',
      uploaded_at: daysAgo(55), page_count: 3,
      fields: [
    ...
    {
      id: templateIds.intake, user_id: DEV_USER_ID, name: 'Client Intake Form',
      file_name: 'Client_Intake_Form.docx', file_type: 'word', file_url: 'dev/templates/intake.docx',
      uploaded_at: daysAgo(40), page_count: 1,
      fields: [
    ...
    {
      id: templateIds.claim, user_id: DEV_USER_ID, name: 'Insurance Claim Form',
      file_name: 'Insurance_Claim_Form.pdf', file_type: 'pdf', file_url: 'dev/templates/claim.pdf',
      uploaded_at: daysAgo(20), page_count: 2,
      fields: [
  ...
  if (r2.error) return NextResponse.json({ error: 'Templates insert failed', detail: r2.error }, { status: 500 });

  // ── Client Documents ──────────────────────────────────────────────────────
  const r3 = await admin.from('client_documents').insert([
    {
  ...
  if (r3.error) return NextResponse.json({ error: 'Documents insert failed', detail: r3.error }, { status: 500 });
